...
final class FastingViewModel: ObservableObject {
    ...
    @Published var customStartTime: Date = Date()
    ...
    func startFast() {
        ...
        isActive = true
        isComplete = false
        timeElapsed = Date().timeIntervalSince(customStartTime)
        progress = min(timeElapsed / targetDuration, 1.0)

        UserDefaults.standard.set(customStartTime, forKey: startTimeKey)
    ...
    func updateStartTime(_ newStart: Date) {
        ...
        UserDefaults.standard.set(newStart, forKey: startTimeKey)

        timeElapsed = Date().timeIntervalSince(newStart)
        progress = min(timeElapsed / targetDuration, 1.0)
    ...
    private func scheduleNotification() {
        ...
        let remaining = max(targetDuration - timeElapsed, 1)
